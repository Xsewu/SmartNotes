...
export default function StudentNotesDashboard({ user }: { user?: { email?: string | null } }) {
  ...
  const fetchNotes = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const formattedNotes: Note[] = data.files.map((file: {
          id: string;
          title: string;
          ...
          pages: 1, // Placeholder
          sharedWith: file.visibility === "PRIVATE" ? "Prywatny" : file.visibility,
          accent: "from-blue-500 to-indigo-400", // Generic accent
          url: file.url
        }));
  ...
  }, [activeCategory, deferredQuery, notes]);
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      setUploadFile(null);
      setUploadTitle("");
      alert("Plik został pomyślnie udostępniony!");
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  <div className="flex -space-x-2">
                    <div className="h-8 w-8 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center text-xs font-bold text-blue-700">AK</div>
                  </div>
                  <div className="text-sm">
                    <p className="font-medium text-slate-900">Udostępnione dla:</p>
                    <p className="text-slate-500">{selectedNote.sharedWith}</p>
                  </div>
                </div>
